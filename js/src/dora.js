...
var DoraModel = widgets.DOMWidgetModel.extend({

    defaults: _.extend({}, widgets.DOMWidgetModel.prototype.defaults, {
        _model_name:   'DoraModel',
        _view_name:    'DoraView',
        _model_module: 'dora',
        _view_module:  'dora',
    ...
    var DoraView = widgets.DOMWidgetView.extend({
        ...
        createCanvas: function(){
            ...
            console.log(doraWidget);
            this.cy = cytoscape({
               container: document.getElementById('cyDiv'),
               layout: {name: 'preset'},
          ready: function(){
            console.log("small dora network ready");
            console.log("ready's notion of this:")
            console.log(this);
            doraWidget.cy = this;
            window.cy = this;  // for easy debugging
            console.log("ready's notion of doraWidget:")
            console.log(doraWidget);
            console.log("calling this.fit")
            //cyWidget.cy.fit(100);
            console.log("--- about to call loadGraph")
            //doraWidget.loadGraph("priyanka.json");
            //doraWidget.loadStyle("style.js");
            //doraWidget.cy.layout({"name": "grid"});
            console.log("    back from loadGraph")
            doraWidget.cy.on("select", function(){
               console.log("calling updateSelectionToKernel, on select");
               doraWidget.updateSelectionToKernel(doraWidget);
               });
            doraWidget.cy.on("unselect", function(){
        ...
        updateSelectionToKernel: function(doraWidget){
           console.log("*** entering updateSelectionToKernel")
           var selectedNodes = doraWidget.cy.nodes(":selected");
           var selectedNodeIDs = selectedNodes.map(function(n){return (n.data("name") )})
           var selectedEdges = doraWidget.cy.edges(":selected");
           ...
           console.log(jsonString);
           doraWidget.model.set("msgFromBrowser", jsonString);
        ...
        loadStyle: function(filename){
           var cyObj = this.cy;
           console.log("doraWidget.loadStyle: " + filename)
